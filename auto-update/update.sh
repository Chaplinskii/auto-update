...
if [ $UPDATED_COUNT -gt 0 ]; then
    ...
    echo "$(date): ⚠ Не удалось определить, какие пакеты были обновлены."
fi

# === Шаг 4: Перезагрузка ===
echo "$(date): 3/3: Перезагружаем систему..."
shutdown -r +1 "Система перезагружается после автоматического обновления"
